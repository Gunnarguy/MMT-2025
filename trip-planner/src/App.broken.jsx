...
// Quick Add Popup - lets users pick day + time slot when adding activities
function QuickAddPopup({ activity, position, tripDays, onAdd, onClose, selectedActivities }) {
  ...
  // For lodging, only show evening/overnight
  const isLodging = activity?.type === 'lodging'
  const availableSlots = isLodging 
    ? [{ id: 'overnight', label: '🛏️ Overnight', time: 'Check-in' }]
    : timeSlots
  
  // Get what's already scheduled for selected day
  ...
  const handleAdd = () => {
    if (selectedDay) {
      onAdd(activity, selectedDay, selectedSlot)
    } else {
      onAdd(activity, null, null) // Add to unscheduled
    }
  }
  ...
  return (
    ...
    >
      <div className="quick-add-header">
        <h4>📅 Add to your trip</h4>
        <button className="close-btn" onClick={onClose}>✕</button>
      </div>
      
      <div className="quick-add-activity-preview">
        <span className="preview-icon">{activity.type === 'lobster' ? '🦞' : activity.type === 'town' ? '⚓' : activity.type === 'foliage' ? '🍁' : activity.type === 'drive' ? '🚗' : '🏨'}</span>
        <div>
          <strong>{activity.name}</strong>
          <span className="preview-location">{activity.location}</span>
        </div>
      </div>

      <div className="quick-add-days">
        <p className="section-label">Which day?</p>
        <div className="day-chips">
          {tripDays.map(day => {
            const dayCount = getDayActivities(day.id).length
            return (
              <button
                key={day.id}
                className={`day-chip ${selectedDay === day.id ? 'selected' : ''} ${dayCount >= 4 ? 'busy' : ''}`}
                onClick={() => setSelectedDay(day.id)}
              >
                <span className="day-num">D{day.id}</span>
                <span className="day-loc">{day.location.split(' ')[0]}</span>
                {dayCount > 0 && <span className="day-count">{dayCount}</span>}
              </button>
            )
          })}
        </div>
      ...
      {selectedDay && (
        <div className="quick-add-slots">
          <p className="section-label">What time?</p>
      ...
      <div className="quick-add-actions">
        <button className="add-unscheduled" onClick={() => onAdd(activity, null, null)}>
          + Add to My Trip (schedule later)
        </button>
        <button 
          className="add-scheduled" 
          onClick={handleAdd}
          disabled={!selectedDay}
        >
          {selectedDay ? `✓ Add to Day ${selectedDay}` : 'Select a day above'}
        </button>
      </div>
    </div>
  )
...
function App() {
  ...
  // Explore category (what content to show in main area)
  const [exploreCategory, setExploreCategory] = useState('lobster')
  
  // Sidebar collapsed state
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  
  // Legacy activeTab for planning/packing screens
  ...
  const [quickAddOpen, setQuickAddOpen] = useState(null) // holds activity being added
  const [quickAddPosition, setQuickAddPosition] = useState({ x: 0, y: 0 })

  // Add activity to selected list (with optional day and time slot)
  const addActivity = (activity, dayId = null, timeSlot = null) => {
    if (!selectedActivities.find(a => a.id === activity.id)) {
      const newActivity = { ...activity, dayId, timeSlot }
      setSelectedActivities([...selectedActivities, newActivity])
      
      // If day is specified, also add to that day's activities
      if (dayId) {
        setTripDays(tripDays.map(day => 
          day.id === dayId 
            ? { ...day, activities: [...day.activities, activity.id] }
            : day
        ))
      }
    }
  ...
  // Open quick-add popup near the button
  const openQuickAdd = (activity, event) => {
    const rect = event.target.getBoundingClientRect()
    setQuickAddPosition({ 
      x: Math.min(rect.left, window.innerWidth - 320), 
      y: rect.bottom + 8 
    })
    setQuickAddOpen(activity)
  }

  // Close quick-add when clicking outside
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (quickAddOpen && !e.target.closest('.quick-add-popup') && !e.target.closest('.add-to-trip-btn')) {
        setQuickAddOpen(null)
      }
    }
  ...
  return (
    <div className={`app-container ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      {/* Quick Add Popup Portal */}
      {quickAddOpen && (
        <QuickAddPopup
          activity={quickAddOpen}
      ...
      <header className="hero compact-hero">
        <div className="hero-content">
          ...
          <div className="hero-meta">
            ...
            <div className="travelers-mini">
              {travelers.map((t) => (
                <span key={t.name} className="traveler-chip" title={t.name}>
                  {t.emoji}
                </span>
              ))}
            </div>
      ...
      {/* Main Layout: Explore + Sidebar */}
      <div className="main-layout">
        
        {/* LEFT: Explore Content Area */}
        <main className="explore-area">
          
          {/* Category Pills - Lightweight Navigation */}
          <nav className="category-nav">
            <div className="category-pills">
              {[
                { id: 'lobster', icon: '🦞', label: 'Lobster' },
                { id: 'foliage', icon: '🍁', label: 'Foliage' },
                { id: 'towns', icon: '⛵', label: 'Towns' },
                { id: 'lodging', icon: '🏨', label: 'Lodging' },
              ].map(cat => (
                <button
                  key={cat.id}
                  className={`category-pill ${exploreCategory === cat.id ? 'active' : ''}`}
                  onClick={() => { setExploreCategory(cat.id); setActiveTab('explore'); }}
                >
            ...
            <div className="utility-pills">
              {[
                ...
                <button
                  key={cat.id}
                  className={`utility-pill ${activeTab === cat.id ? 'active' : ''}`}
          ...
          {/* EXPLORE CONTENT - Shows based on category */}
          {activeTab === 'explore' && (
            <div className="explore-content">
              
              {/* 🏨 LODGING */}
              {exploreCategory === 'lodging' && (
                        ...
                        <div key={lodge.id} className={`lodging-card ${lodge.type === 'Private Home' ? 'special' : ''} ${selected ? 'selected' : ''}`}>
                          ...
                          <div className="card-actions">
                            ...
                            <button 
                              ...
                              onClick={(e) => selected ? removeActivity(lodge.id) : openQuickAdd({
                                id: lodge.id,
                                type: 'lodging',
                                name: lodge.name,
      ...
      {/* 🦞 LOBSTER - in explore content */}
      {exploreCategory === 'lobster' && (
        <>
              ...
              {lobsterGuide.topSpots.map((spot, idx) => {
                ...
                <div key={spot.name} className={`lobster-card ${idx === 0 ? 'top-pick' : ''} ${selected ? 'selected' : ''}`}>
                  ...
                  <div className="card-actions">
                    ...
                    <button 
                      className={`add-to-trip-btn ${selected ? 'added' : ''}`}
                      onClick={(e) => selected ? removeActivity(activityId) : openQuickAdd({
                        id: activityId,
                        type: 'lobster',
                        name: spot.name,
                        location: spot.location,
