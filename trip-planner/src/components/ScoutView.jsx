...
import { MoneyBlock, moneyFor } from "./YourMoney";
import { DEFAULT_BUDGET, MonthBlock, estimateMonth } from "./YourMonth";
import ScoutMatrix from "./ScoutMatrix";
  ...
  return (
    <>
      <div className="page-head">
        ...
        <h1>Fifteen towns, one table</h1>
        <p>
          Every town this trip drives through, measured the same way and lined up against Campbell:
          what the median house costs to own on your income, what your actual monthly bills become,
          how much snow falls and how short December gets, how far the ER, Costco, the airport, Mom
          and Stryker are, and how safe it is. Sort any column, pin the contenders, then open a town's
          full workup underneath for the prose and the sources. The same towns are a pin layer on the Map tab.
        </p>
      </div>

      <ScoutMatrix
        towns={RELOCATION_TOWNS}
        home={SCOUT_CAMPBELL}
        moneyOf={moneyOf}
        monthOf={monthOf}
        matchOf={matchOf}
      ...
      {SCOUT_CAMPBELL && (
        <section style={{ marginBottom: "var(--s-6)" }}>
          <div className="eyebrow" style={{ color: "var(--accent)", marginBottom: "0.35rem" }}>
            Deep dives · where you live now, the baseline
          </div>
          <p className="muted" style={{ fontSize: "var(--t-sm)", marginBottom: "var(--s-3)" }}>
            Below the matrix, every town gets the long form: the one-liner, the scorecard, your money and your
            month, the climate strip, and the full workup with sources. Campbell first, quantified the same way.
          </p>
