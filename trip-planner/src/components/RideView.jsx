...
import FlightDeck from "./visuals/FlightRunway";
    ...
    <>
      ...
      <section className="section">
        ...
        {flights.map((f) => {
          ...
          return (
            <div className="ride-flight" key={f.id}>
              <FlightDeck flightId={f.id} flight={f} />
              {verdict && (
                <div className={`ride-verdict ride-verdict--${verdict.tone}`}>
                  {verdict.text}
                </div>
              )}
              <details className="ride-edit">
                <summary>Edit {f.number}: times, seats and record locators (if American changes something)</summary>
                <div className="ride-edit-body">
                  <div className="fld-grid">
                    {FLIGHT_FIELDS.map((def) => (
                      <Field key={def.key} def={def} value={f[def.key]} onChange={(k, v) => setF(f.id, k, v)} />
                    ))}
                  </div>
                  {(f.passengers || []).map((pax, i) => (
                    <div className={`ride-pax${i === 0 ? " is-first" : ""}`} key={pax.name || i}>
                      {PASSENGER_FIELDS.map((def) => (
                        <Field key={def.key} def={def} value={pax[def.key]} onChange={(k, v) => setP(f.id, i, k, v)} />
                      ))}
                    </div>
                  ))}
                  <p className="fld-why">Saved on this device only. The deck above, the Today tab and the day pages all follow these times.</p>
                </div>
              </details>
            </div>
          );
        ...
        <div className="stack" style={{ gap: "var(--s-3)", marginTop: "var(--s-4)", marginBottom: "var(--s-4)" }}>
          <h3 className="ride-playbook-title">Flight playbook</h3>
